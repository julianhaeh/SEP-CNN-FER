...
MODEL_CONFIGS = {
    ...
    "VGG13_TaperedClassifierOrignal" : {
                "feature_config": original_feature_config,
                "classifier_config": [
    ...
    "VGG13_E_And_C" : {
        ...
        "classifier_config" : removed_onelayer_classifier
    },

    "VGG13_E_And_IncreasedCovDepth" : {

            "feature_config" : [
                                    {'type': 'conv', 'out': 96, 'k': 3, 's': 1, 'p': 1},
                                    {'type': 'norm'}, # New Layer
                                    {'type': 'act'},
                                    {'type': 'conv', 'out': 96, 'k': 3, 's': 1, 'p': 1},
                                    {'type': 'norm'}, # New Layer
...
MODELS_TO_TRAIN = [
    ...
    #( "VGG_Removed1BlockInFeatureLayer_B", MODEL_CONFIGS["VGG_Removed1BlockInFeatureLayer_B"]),
    ...
    #( "VGG13_B_And_HalfClassifier", MODEL_CONFIGS["VGG13_B_And_HalfClassifier"]),
    ...
    #( "VGG13_HalfClassifier", MODEL_CONFIGS["VGG13_HalfClassifier"]),
    #( "VGG13_TaperedClassifierSmall", MODEL_CONFIGS["VGG13_TaperedClassifierSmall"]),
    #( "VGG13_QuarterClassifier", MODEL_CONFIGS["VGG13_QuarterClassifier"]),
    #( "VGG13_TaperedClassifierOrignal", MODEL_CONFIGS["VGG13_TaperedClassifierOrignal"])

    #( "VGG13_E_And_C", MODEL_CONFIGS["VGG13_E_And_C"]),
    #( "VGG13_E_And_IncreasedCovDepth", MODEL_CONFIGS["VGG13_E_And_IncreasedCovDepth"]),
    #( "VGG13_E_And_A", MODEL_CONFIGS["VGG13_E_And_A"]),
    #( "VGG13_E_And_B", MODEL_CONFIGS["VGG13_E_And_B"]),
]
# --- HELPER FUNCTIONS ---
...
def plot_multiple_runs(histories_list, seeds_list, title, filename, ylabel="Value"):
    """Plot multiple runs on the same graph with different colors."""
    plt.figure(figsize=(12, 7))

    colors = ['#1f77b4', '#ff7f0e', '#2ca02c']  # Blue, Orange, Green

    for idx, (history, seed) in enumerate(zip(histories_list, seeds_list)):
...
def train_evaluate_pipeline(model_config, config_name="", seed=42):
    """
    ...
    trainDataLoader = DataLoader(OurDatasetTuning(section='architecture', split='train'), batch_size=BATCH_SIZE, shuffle=True)
    valDataLoader = DataLoader(OurDatasetTuning(section='architecture', split='valid'), batch_size=BATCH_SIZE, shuffle=False)
...
def run_experiments():
    ...
    for config_name, config_model in MODELS_TO_TRAIN:
        ...
        # Lists to collect results from multiple runs
        ...
        all_test_loss_histories = []
        all_acc_histories = []

        # Store the results from the last run for confusion matrix
        last_model = None
        last_y_true = None
        last_y_pred = None

        # Run the experiment NUM_RUNS_PER_CONFIG times with different seeds
        for run_idx in range(NUM_RUNS_PER_CONFIG):
            seed = FIXED_SEEDS[run_idx]
            print(f"\n   >>> Run {run_idx + 1}/{NUM_RUNS_PER_CONFIG} with seed {seed}")
            ...
            all_final_accuracies.append(acc_hist[-1])
            all_final_test_losses.append(test_loss[-1])
            all_final_train_losses.append(train_loss[-1])

            # Store ALL histories for plotting
            all_train_loss_histories.append(train_loss)
            all_test_loss_histories.append(test_loss)
            all_acc_histories.append(acc_hist)

            # Store last run for confusion matrix
            last_model = model
            last_y_true = y_true
            last_y_pred = y_pred

            print(f"   Run {run_idx + 1} Final Accuracy: {acc_hist[-1]:.2f}%")

        # Calculate statistics
        mean_acc = np.mean(all_final_accuracies)
        std_acc = np.std(all_final_accuracies)
        mean_test_loss = np.mean(all_final_test_losses)
        std_test_loss = np.std(all_final_test_losses)
        mean_train_loss = np.mean(all_final_train_losses)
        std_train_loss = np.std(all_final_train_losses)

        print(f"\n   === Results for {config_name} ===")
        print(f"   Final Accuracy: {mean_acc:.2f}% ± {std_acc:.2f}%")
        print(f"   Final Test Loss: {mean_test_loss:.4f} ± {std_test_loss:.4f}")
        print(f"   Final Train Loss: {mean_train_loss:.4f} ± {std_train_loss:.4f}")

        # Plot all runs together
        # 2. Plot Training Loss (all runs)
        plot_multiple_runs(
            all_train_loss_histories,
            FIXED_SEEDS,
            f"Training Loss - {config_name} (All Runs)",
            f"Experiments/Plots/Train_Loss_{config_name}.png",
            ylabel="Loss"
        )

        # 3. Plot Test Loss (all runs)
        plot_multiple_runs(
            all_test_loss_histories,
            FIXED_SEEDS,
            f"Test Loss - {config_name} (All Runs)",
            f"Experiments/Plots/Test_Loss_{config_name}.png",
            ylabel="Loss"
        )

        # 4. Plot Test Accuracy History (all runs)
        plot_multiple_runs(
            all_acc_histories,
            FIXED_SEEDS,
            f"Test Accuracy - {config_name} (All Runs)",
            f"Experiments/Plots/Accuracy_{config_name}.png",
            ylabel="Accuracy (%)"
        )

        # 5. Plot Confusion Matrix
        cm_tensor = compute_confusion_matrix_torch(last_y_true, last_y_pred, num_classes=6)
        ...
        if SAVE_MODELS:
            torch.save(last_model.state_dict(), f"Experiments/Models/{config_name}_Acc{int(mean_acc)}.pth")

        # Log mean and std to file
        with open(LOG_FILE, "a") as f:
            f.write(f"Experiment: {config_name}, Accuracy: {mean_acc:.2f}% std {std_acc:.2f}%, Test-Loss: {mean_test_loss:.4f} std {std_test_loss:.4f}, Train-Loss: {mean_train_loss:.4f} std {std_train_loss:.4f}\n")
